...
class Bar(Window):
    def __init__(
        ...
        mem = Fabricator(
            stream=True,
            poll_from=f"""/home/{USER_NAME}/.config/nothing/scripts/memory""",
        )
        mem.connect("changed", self.sigh)

        cpu = Fabricator(
            stream=True,
            poll_from=f"""/home/{USER_NAME}/.config/nothing/scripts/cpu""",
        )
        cpu.connect("changed", self.cpu)

        batt = Fabricator(
            stream=True,
            poll_from=f"""/home/{USER_NAME}/.config/nothing/scripts/battery""",
        )
